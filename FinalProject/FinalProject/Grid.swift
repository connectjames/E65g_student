...
public extension Grid {
    ...
    public static func parseToCellInitializer(cellData: [String:[[Int]]]) -> (GridPosition) -> CellState {
        if(cellData.count > 0) {
            //let cborn:[[Int]] = [[Int]]()
            var calive:[[Int]] = [[Int]]()
            //let cdied = cellData["died"] ?? []
            for (k ,v) in cellData {
                print("For: \(k)")
                for i in v {
                    calive.append(i)
            ...
            //print("CellDtaa: \(cellData)")
            //print("Born: \(cborn.count) Alive: \(calive.count) Died: \(cdied.count)")
            func cellInitializer(pos: GridPosition) -> CellState {
                //first return should exit
                let posA = [pos.row, pos.col]
                //if cborn.contains(where:  {$0 == posA}) { return .born }
                if calive.contains(where: {$0 == posA}) { return .alive }
                //if cdied.contains(where: {$0 == posA}) { return .died }
                // if none found
                return .empty
            }
